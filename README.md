...
## 초록
대규모 언어 모델(LLM)이 멀티모달 입력과 에이전트 기반 도구 호출을 지원하면서, 공격 표면이 텍스트 단일 채널을 넘어 이미지·오디오 채널 및 서비스 로직 계층으로 확장되고 있다. 본 프로젝트에서는 이러한 다층적 공격 벡터를 통합적으로 탐색하는 LLM 퍼징 시스템을 제안한다. HarmBench 데이터셋을 시드 코퍼스로, GPTFuzzer의 LLM 기반 변이 연산자를 mutation 엔진으로 채택하되, 코드 커버리지 대신 정책 위반 스코어링을 피드백 신호로 활용하여 적대적 입력을 자동 생성한다. 멀티모달 공격 경로로는 Base64 인코딩 등을 통해 유해 프롬프트를 이미지 및 오디오에 은닉하여 안전 필터를 우회한다. 평가 환경으로 Gemini 2.5 Flash 기반 쇼핑몰 챗봇 서비스를 구축하였으며, 공격자 모델은 해당 채팅 인터페이스에 접근 가능한 모든 외부 사용자로 설정하였다. BurpSuite를 통한 패킷 캡처와 모델의 CLI 명령 호출을 결합하여 정책 우회 및 비즈니스 로직 악용을 실증하였다. 본 연구는 실제 서비스 환경에서의 LLM 안전성 평가를 텍스트·멀티모달·에이전트의 세 축으로 확장한 실질적 레드팀 자동화 프레임워크를 제시한다
